...
// DeleteModelData 删除 model 相关的 meta 和 store 中的数据，用于 gc，只能被系统调用
//
//	@param dataItemList require field: id and type
func DeleteModelData(dataItemList []basicdo.DataItemDo) error {
...
func deleteMeta(dataItemMap map[string][]basicdo.DataItemDo) error {
	for _, dataItemList := range dataItemMap {
		err := repo.ModelRepo.DeleteBatch(basicdo.GetDataItemIdList(dataItemList))
		if err != nil {
			return err
		}
	}
