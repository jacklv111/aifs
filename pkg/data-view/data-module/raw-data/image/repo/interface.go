...
type ImageRawDataRepoInterface interface {
	// CreateBatch 批量插入 image 相关数据。如果该数据已经存在，则将其 id 替换成存在的 item id。判断数据是否相同使用 hash。
	//
	//  @param dataItemDoList
	//  @param imageExtDoList
	//  @param imageScoreDoList
	//  @return error
	CreateBatch(dataItemDoList []basicdo.DataItemDo, imageExtDoList []do.ImageExtDo, imageScoreDoList []do.ImageScoreDo) error

	// FindExistedByHash 找出 imageExtDoList 中 hash code 存在的 data item
	//
	...
	//  @return size
	//  @return err
	GetTotalDataSize(dataItemIdList []uuid.UUID) (size int64, err error)
}
